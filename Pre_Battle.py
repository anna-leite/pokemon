    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                hitbox_image_height = 115

                """
                Ajouté la gestion des clics
                pour chaque pokemon
                """

                # clic sur premier pokemon
                if 100 <= mouse_x <= 250 and 220 <= mouse_y <= (200 + hitbox_image_height):
                    print("premier")

                # clic sur second pokemon
                elif 100 <= mouse_x <= 250 and 220 <= mouse_y <= (330 + hitbox_image_height):
                    print("second")

                # clic sur troisieme pokemon
                elif 100 <= mouse_x <= 250 and 220 <= mouse_y <= (465 + hitbox_image_height):
                    print("troisieme")
        ...
        # Draw random Pokémon
        screen.blit(random_pokemon_image, (600, 100))
        random_pokemon_name = font.render(random_pokemon.get_name(), True, COLOURS['BLACK'])
        screen.blit(random_pokemon_name, (600, 50))
        ...
        screen.blit(rp_types_text, (600, 80))

        # Draw the text box
        draw_text_box(TEXT['NEW_OPPONENT'], 250, 180)
...
#            print(y)
        ...
        for i, pokemon in enumerate(fighting_team):
            # Pokémon image
            pokemon_image = pygame.image.load(f"./Assets/Images/Pokemon/{pokemon}_front.png")
            images.append(pokemon_image)

        # premiere image
        screen.blit(images[0], (100, 220))

        # seconde image
        screen.blit(images[1], (100, 350))


        #  troisieme image
        screen.blit(images[2], (100, 480))
